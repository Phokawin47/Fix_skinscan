...
<style>
.container,.max-w-6xl{overflow:visible!important;transform:none!important}
.section-products,.products-grid{position:relative;overflow:visible!important}

.product-card{
  position:relative;z-index:0;overflow:visible;
  transition:transform .2s ease,box-shadow .05s ease;
  isolation:isolate;padding-bottom:3.5rem;
}
.product-body{transition:opacity .15s linear}
.product-img{height:9rem;display:flex;align-items:center;justify-content:center;overflow:hidden}
...
/* ซ่อนรายละเอียดค่าเริ่มต้น */
.product-card .product-desc{
  display:none;max-height:3.6em;overflow:hidden;position:relative;
}
.product-card .product-desc::after{
  content:"";position:absolute;left:0;right:0;bottom:0;height:1.4em;
  background:linear-gradient(to top,#fff,transparent);
}

/* เมื่อกด More (mode-more) -> เปิดโหมดเต็มค้างไว้ */
.product-card.mode-more{
  z-index:9999;transform:scale(1.05);box-shadow:0 16px 40px rgba(0,0,0,.25);
}
.product-card.mode-more .product-body{opacity:0;visibility:hidden}
.product-card.mode-more .product-flyout{
  opacity:1;transform:scale(1.1) translateY(-20px);
  pointer-events:auto;z-index:200;
}
.product-card.mode-more img{transform:scale(1.08)}
.product-card.mode-more .product-desc{display:block;max-height:none}
.product-card.mode-more .product-desc::after{display:none}

/* ปุ่ม More/Less */
.more-toggle{
  position:absolute;bottom:.8rem;right:50%;transform:translateX(50%);
  z-index:500;padding:.45rem .9rem;border-radius:9999px;
  font-size:.85rem;font-weight:500;color:#fff;
  background:linear-gradient(135deg,#2563eb,#1d4ed8);
  border:none;box-shadow:0 2px 6px rgba(0,0,0,.25);
...
<body>
  ...
  <main class="max-w-6xl mx-auto px-4 md:px-6 lg:px-0 py-10">
    <div class="w-full flex justify-center mb-6">
      <div class="px-6 py-2 rounded-full bg-gray-100 text-gray-800 text-sm font-semibold shadow-sm">Search and filter</div>
    </div>
    ...
    <section class="rounded-3xl border border-gray-200 bg-white shadow-sm p-4 md:p-6 section-products">
      <form method="GET" action="{{ route('search') }}" class="mb-6" id="searchForm">
        <div class="flex items-center gap-2">
          <div class="flex-1 relative">
            <input name="q" value="{{ request('q') }}" placeholder="ค้นหาชื่อผลิตภัณฑ์"
            ...
            <button type="button" id="advancedBtn"
              class="absolute right-11 top-1/2 -translate-y-1/2 text-sm px-3 py-1 rounded-full border border-gray-300 hover:bg-gray-50">Advanced</button>
            <button type="submit"
        ...
        <div id="advancedPanel" class="{{ request()->hasAny(['ingredient','acneType']) ? '' : 'hidden' }} rounded-2xl border border-gray-200 p-4 mt-4 bg-gray-50">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div class="flex items-end gap-2">
              <a href="{{ route('search') }}" class="h-10 px-4 rounded-lg border border-gray-300 hover:bg-white inline-flex items-center">ล้างตัวกรอง</a>
      ...
      <script>
        document.getElementById('advancedBtn')?.addEventListener('click',()=>document.getElementById('advancedPanel')?.classList.toggle('hidden'));
      </script>
  ...
  :class="(enabled ? 'product-card mode-more ' : 'product-card ') + 'rounded-2xl border border-gray-200 p-3 md:p-4 bg-white'">

          <!-- ปุ่ม More/Less -->
          <button type="button" class="more-toggle"
        @click.stop="enabled = !enabled"
        x-text="enabled ? 'Less' : 'More'"></button>
          ...
          <div class="product-body">
            ...
            <div class="text-sm text-gray-700">
              ...
              <!-- ย่อหน้าจะโผล่เฉพาะเมื่อ mode-more -->
